...
MRH_SESSION = "a35a5cfcd7bd05f1b2abf0388278ceff" ## Session ID (This will be changed once moved to ORC Probably)
#XXX
...
sheet_name = "Malware_EP_Crowdstrike.csv"
# m_job_id = "1478723"  ## From MSV 
m_job_id = "2515622"  ## From MSV 
...
def risk_ranking(Impact, Detect, Prevent):
    ...
    medium_list = ["Malicious File Transfer", "Information Gathering", "Brute Force", "Email Exfil", "Evasion Technique", "ICMP Tunnel-based",
                   "DNS Tunnel-based",
...
with open(f'{sheet_name}', mode='w', encoding="utf-8") as csv_file:
# with open(f'JobID_{m_job_id}_AS.csv', mode='w', encoding="utf-8") as csv_file:
    ...
    csv_header_names = ["Action_ID", "Action_Name", "Action_Description",  # Headers for CSV Document
                        "Detected", "Prevented", "SecTech_Component", "Risk Level"
                        ]
    # csv_header_names = ["Action_ID", "Action_Name", "Action_Description",  # Headers for CSV Document
...
#     json.dump(job_raw_data, outfile)
    ...
    for i in job_steps_data:
        ...
        for j in job_action_data:
            ...
            if j.get('detected'):
                # Combine both host_events and integration_events into a single list
                all_events = j.get('host_events', []) + j.get('integration_events', [])
                ...
                for event in all_events:
                    endpoint_product = event.get('endpoint_product')
                    if endpoint_product and endpoint_product.get('product') == 'Falcon Endpoint Security':
                        sectech_component = 'CrowdStrike'
                    # if endpoint_product and endpoint_product.get('product') == 'Windows Defender AV':
                    #     sectech_component = 'Microsoft Defender'
                        break  # Stop after finding the desired product

                if sectech_component is None:
                    sectech_component = "None"
            ...
            #     product_names = set()  # Use a set to store unique product names

            #     # Loop through all events to find the vendor value
            #     for event in all_events:
            #         endpoint_product = event.get('endpoint_product')
            #         if endpoint_product and 'product' in endpoint_product:
            #             product_name = endpoint_product['product']
            #             product_names.add(product_name)

            #     if product_names:
            #         sectech_components = [vendor_mapping.get(product_name, 'Unknown') for product_name in product_names]
            #     else:
            #         sectech_components = ["None"]

            #     print(sectech_components)
            # else:
            #     sectech_components = ["None"]
            ...
            sampledata = summary, action_Name, action_description, res_detected, res_blocked, sectech_component, risk
