...
JIRA_SEARCH_JQL = "project = SECVAL AND issuetype = 'Attack Simulation' AND 'Epic Link' = SECVAL-9009"
...
job_url = "https://app.validation.mandiant.com/v2/jobs/" + Job_ID + ".json?pretty&exclude=" + filter_data
headers = {
...
def check_label(k): ## Use jira API for labels
    label_url = JIRA_DEV_URL_LABEL+ "{}".format(k)
...
def add_labels(key, change):
    addlabel_url = JIRA_DEV_URL_LABEL + "{}".format(key) 
...
def remove_labels(key, change):
        remlabel_url = JIRA_DEV_URL_LABEL + "{}".format(key) 
...
## Jira Items
jira_response = requests.request("GET", JIRA_DEV_URL + "search?jql=" + JIRA_SEARCH_JQL + JIRA_MAX_RESULTS, headers=JIRA_HEADERS, verify=False, auth=JIRA_AUTH, cookies=COOKIES)
