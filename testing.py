...
MRH_SESSION = "2d0775237b946a31a42c6f4d9d0ef79b"
#XXX
...
#XXX
EpicLink = "SECVAL-2359"  ## Required Jira Value 
#XXX
...
url = "https://code-jira.am.sony.com/rest/api/2/issue"
        ...
        for j in job_action_data:
            ...
            payload = json.dumps({
                "fields": {
                    ...
                    "customfield_22301": f"{assessment_name}",  ##Assessment name
                    "customfield_16404": {"value": gsirt1, "child": {"value": gsirt2}},  # GSIRT Sytem field
                    "customfield_22302": {"value": risk},  # Risk Priority field
                    "customfield_22303": [{"value": control_gap_det}, {"value": control_gap_pre},
                                          {"value": control_gap_ale}],
                    "assignee": {"name": Assignee},
                    "customfield_22304": {"value": "Internal"},  # Remediation Owner field
                    "customfield_22305": [{"value": control_a}],  # Control Area field
                    "issuetype": {
...
for i in job_steps_data:
    ...
    for j in job_action_data:
        ...
        ## Do not print the action that has only "alerting" as the control gap
        if j["detected"] and j["blocked"] and not j["alerted"]:
            continue
        ...
        if summary_dep in ALL_TICKET_INFO:  # If Match found - link tickets
            ...
            payload = json.dumps({
                "fields": {
                    ...
                    "customfield_15501": f"{EpicLink}",                    
                    "customfield_23101": {"value": GSIRT_pro},
                    "priority": {"name": f"{risk}"},
